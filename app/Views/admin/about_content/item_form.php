...
$isEdit  = $item !== null;
$action  = $isEdit
    ? '/admin/about-content/item/update/' . $item['id']
    : '/admin/about-content/item/create/' . $section;
$backUrl = '/admin/about-content/section/' . $section;
?>
...
<div class="card" style="max-width:720px;">
    ...
    <div class="card-body">
        <form method="post" action="<?= $action ?>">
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    <label class="form-label">Year <small class="text-muted">(optional)</small></label>
                    <input type="text" name="year" class="form-control form-control-sm"
                           value="<?= esc($item['year'] ?? '') ?>" placeholder="e.g. 2018">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Icon <small class="text-muted">(e.g. bi-star)</small></label>
                ...
                <div class="col-12">
                    ...
                    <div class="img-add-tabs mb-2">
                        <button type="button" class="img-add-tab active" data-panel="ac-tab-upload">
                            <i class="bi bi-upload"></i> Upload
                        </button>
                        <button type="button" class="img-add-tab" data-panel="ac-tab-library">
                            <i class="bi bi-images"></i> Library
                        </button>
                        <button type="button" class="img-add-tab" data-panel="ac-tab-url">
                            <i class="bi bi-link-45deg"></i> URL
                        </button>
                    </div>

                    <div id="ac-tab-upload">
                        <div class="d-flex align-items-center gap-2">
                            <input type="file" id="acFileInput" class="form-control form-control-sm"
                                   accept="image/jpeg,image/png,image/webp" style="max-width:260px;">
                            <span id="acUploadStatus" class="form-text"></span>
                        </div>
                        <div class="form-text text-muted mt-1">Uploaded to media library · auto-converted to WebP</div>
                    </div>
                    <div id="ac-tab-library" style="display:none;">
                        <button type="button" class="btn btn-sm btn-outline-secondary w-100" id="acOpenPickerBtn">
                            <i class="bi bi-images me-1"></i> Browse Media Library
                        </button>
                    </div>
                    <div id="ac-tab-url" style="display:none;">
                        <div class="d-flex gap-2">
                            <input type="text" id="acExtUrlInput" class="form-control form-control-sm"
                                   placeholder="https://example.com/image.jpg">
                            <button type="button" class="btn btn-sm btn-primary" id="acAddExtUrlBtn" style="white-space:nowrap;">Add</button>
                        </div>
                    </div>

                    <input type="hidden" name="image_url" id="acImageUrl" value="<?= esc($item['image_url'] ?? '') ?>">
                </div>
            </div>
...
<div class="modal fade" id="acPickerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content" style="background:var(--card-bg);border-color:var(--card-border);">
            ...
            <div class="modal-body px-4 py-3" id="acPickerBody" style="min-height:70vh;">
...
<script>
document.body.appendChild(document.getElementById('acPickerModal'));
...
document.querySelectorAll('.img-add-tab[data-panel]').forEach(function(btn) {
    btn.addEventListener('click', function() {
        ...
        ['ac-tab-upload','ac-tab-library','ac-tab-url'].forEach(function(id) {
...
function acSetUrl(url) {
    if (!url) return;
    document.getElementById('acImageUrl').value = url;
    document.getElementById('acFileInput').value = '';
}

// Upload tab — AJAX upload to library
document.getElementById('acFileInput').addEventListener('change', function() {
    var file = this.files[0];
    if (!file) return;
    var status = document.getElementById('acUploadStatus');
    ...
    xhr.onload = function() {
        try {
            ...
            if (res.success) { acSetUrl(res.url); status.textContent = '✓ Uploaded'; status.style.color = 'var(--accent)'; }
            else { status.textContent = 'Error: ' + (res.error || 'Upload failed.'); status.style.color = '#ef4444'; }
...
// URL tab
document.getElementById('acAddExtUrlBtn').addEventListener('click', function() {
    var url = document.getElementById('acExtUrlInput').value.trim();
    if (url) acSetUrl(url);
});

// Library picker
document.getElementById('acOpenPickerBtn').addEventListener('click', function() {
    bootstrap.Modal.getOrCreateInstance(document.getElementById('acPickerModal')).show();
    acLoadPicker('about');
});

function acLoadPicker(folder) {
    var body = document.getElementById('acPickerBody');
    body.innerHTML = '<div class="text-center text-muted py-5"><span class="spinner-border spinner-border-sm me-2"></span> Loading…</div>';
    fetch('/admin/media/picker?folder=' + encodeURIComponent(folder || 'about'), { headers: {'X-Requested-With':'XMLHttpRequest'} })
    .then(function(r) { return r.text(); })
    .then(function(html) { document.getElementById('acPickerBody').innerHTML = html; })
    .catch(function() { document.getElementById('acPickerBody').innerHTML = '<div class="text-center text-muted py-5">Failed to load.</div>'; });
}

document.getElementById('acPickerBody').addEventListener('click', function(e) {
    var folder = e.target.closest('.picker-folder');
    if (folder) { acLoadPicker(folder.dataset.folder); return; }
    var item = e.target.closest('.picker-item');
    if (item) { acSetUrl(item.dataset.url); bootstrap.Modal.getInstance(document.getElementById('acPickerModal'))?.hide(); }
});

document.getElementById('acPickerBody').addEventListener('change', function(e) {
    ...
    xhr.onload = function() {
        try { var res = JSON.parse(xhr.responseText);
            if (res.success) { acSetUrl(res.url); bootstrap.Modal.getInstance(document.getElementById('acPickerModal'))?.hide(); }
